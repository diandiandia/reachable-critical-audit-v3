...
- H3 异步生命周期竞态 (CWE-416)
  （v3.43, SWR-V3.43-005, 提示级）kernel 形态锚点: 任务退出竞态（退出
  路径与回调并发）、timer/延迟回调持引用、调度器状态复用（RBTree 双插入
  形态）、异步子系统对象生命周期（io_uring 形态）——2025 年 UAF 族六大
  CVE 与 H3 检测要点的对应形态。
- H4 跨进程信任边界破坏 (CWE-20+89/78)（含 reply 通道族 W6 §15.4、GIT_CONFIG/env 重定向族 W6 §12.7）
  初始化时序注入面 (v3.11, SWR-V3.11-012): 启动参数/初始化前到达的输入在安全
  配置生效前被消费（启动窄窗/冷启动注入/初始化时序竞态）——输入到达时机与
  安全门生效时序的先后关系是判定前提
  ...
  site-isolation/资源归因 (v3.24, SWR-V3.24-001): 跨进程资源（纹理/缓存/下载/导航目标/worker 等）绑定到错误 origin 即信任边界破坏——检查资源创建与归因的 origin 上下文一致性
...
## R2 进行中结论注入（v3.23, SWR-V3.23-006，提示级）

主代理派发本任务书时须在任务书附带「R2 进行中结论」段：与本假说相关 surface 的
R2 筛选进行中结论（keep/drop 条目 + 理由）；R2 尚未完成时标注「R2 未出结论」。
R4 与 R2 并行时此段防止同一事实双通道矛盾至 gate ③b 才暴露（跨通道契约对账义务，
并行审计实录）。无 R2 结论不构成任务书缺陷（提示级）。
