...
4. 语义族与项目平台不匹配
4.5. **位域/ordinal 越界判据的证据义务（v3.40, SWR-V3.40-004）**: 判
   「位域 ordinal 直索引定长枚举数组越界」前必须 Read 枚举常量声明行核对
   (a) bit 长度（如 `TYPE(PERMISSION.BITS, 2)` 第二参）与 (b) 枚举值数——
   声明行 file:line 写入 note。bit 范围 ⊆ 枚举值数时该判据不成立
   （实测形态: 「3bit 索引 4 值枚举」声称与源码 2bit 声明不符, 真实溢出
   在同族另一格式类的 nid 组合域）
5. **『防御已到位』**（gate 默认开启/默认 token 随机/默认白名单等）——drop 前必须
   ...
   两方向裁决不可互套（实测形态: 同批次鉴权 gate 默认关假设 13 条全 keep,
   方向判定缺失即系统性误 drop）
...
## 逻辑镜像枚举（v3.11, SWR-V3.11-013/014）
多语言目标（语言清单 ≥2）下，同逻辑输入面可能在多语言各有实现（跨语言桥接的
编解码/协议处理/通道实现族）——keep 判定时须核实该逻辑面的**全部语言实现**
是否覆盖（任一实现无对应假设即补假设; 主代理在 merge 阶段输出 mirror_candidates
提示供对照）。任一实现的 sink 同构缺陷被遗漏 = 整个缺陷族漏报一个面。
