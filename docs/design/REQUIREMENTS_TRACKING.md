...
| SWR-V3.7-009 | 确认问题全集 = R3 REACHABLE ∪ R4 confirmed findings（High/Medium 申报值归一化并入问题清单；Low 留附录 B；r3_link 指向候选的同事实条目不重复列+尾注去重） | 已完成 | _confirmed_issues/_render_problem_list/_render_problem_details（tools/batch_verify.py）+ tests/test_v37_report.py:11-14 |
| SWR-V3.7-010 | R4 finding 分级 = 申报值归一化（High→高/Medium→中/Low→低；缺失非法回退机械映射）；R4 详情标注「无 R3.5 独立复核」 | 已完成 | _r4_severity（tools/batch_verify.py）|
...
## 系统需求（REQ-V3.35）（共 2 条）

> 手工追加段（2026-09-08，随 v3.36 补登记）。案例支撑：用户裁定 SKILL.md
> 只保留审计设计内容——删除非审计数据（开发侧纪律/历史增量段冗余）并重构。

| 编号 | 需求 | 状态 | 证据 |
|---|---|---|---|
| REQ-V3.35-001 | SKILL.md 净化: 38 增量段迁档 + 17 条规范性归位正文 + 只留审计内容 | 已完成 | SKILL.md + docs/history/SKILL_INCREMENTS.md |
| REQ-V3.35-002 | 开发侧纪律（义务入库三问/修改自检）移出 SKILL.md（属 skill-optimizer） | 已完成 | 用户裁定 + 提交 62859aa |

## 系统需求（REQ-V3.36）（共 2 条）

> 手工追加段（2026-09-08）。案例支撑：Caddy 阶段 6 验收审计 R1 合并现场
> ——35/70 面 trust_boundary 被关键词映射器静默改写（规范枚举被当自由文本
> 映射），审计地基数据一半失真；违反不自动改写纪律。
...
| SWR-V3.44-009 | SKILL.md 同项目多批次开题四步（五年窗口 recon → R1 → 计划 → R2-R6, 用户裁定） | 已实现 |
