...
## 修订记录（v3.3.2, SWR-V3.3.2-070）

- **REQ-V3.2-021 增补**（2026-08-19, 依据七项目批次复盘 §27 A2）：
  原条文"复活成功 → 候选回 R3 重验（附复活者证据），不直接改 verdict"追加一句：
  **"重验改判 REACHABLE 且 grade≥edge_proven → 强制入 R3.5 证伪池（放行方向的
  独立复核义务，gate post_resurrect_refutation 强制）"**。
  动机：本批 8 个复活改判 REACHABLE 全部跳过 R3.5 证伪——机制存在（export
  refutation pool 天然覆盖）但义务无强制力。
