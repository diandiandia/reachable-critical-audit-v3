...
## 0. 设计第一原则：通用性（2026-08-17 确立，最高优先）

**本 skill 是通用型代码审计 skill**。任何语言、任何项目形态（application/library/hybrid/infra）、任何平台的任意代码库均可审。所有能力必须来自通用机制，**禁止因为审过某个项目而对它做专门优化**。

三禁止（违反即设计缺陷）：
1. 项目名/目录结构/专属 API 名不得进入运行时资产（签名 grep、任务书例证、harness 模板、先例/清单正文）
2. 运行时机制不得依赖单一语言特征（语言内容按 `lang` 分派或入语言手册）
3. 历史项目目录不得做运行时锚定（回归锚点仅限 tests/ fixture）

两段式经验提炼：具体发现 → 去项目化（抽象到 CWE 类/语言无关模式/通用检查步骤）→ 入库。项目名仅允许出现在追溯字段。

验收约束：每版本验收必须包含至少一个**未审计过的新项目**，防止机制向历史项目收敛。

> 来源：mbedtls 审计复盘（签名库 13/13 携带历史项目残留、verifier 任务书 Python 思维定式、harness 按历史战役配置、R0 冒烟仅对历史 fixture 有意义）。修复方案 = v3.2.2 问题域 P-A（资产去项目化）。
> 本文档其他章节若与本原则冲突，以本原则为准。
