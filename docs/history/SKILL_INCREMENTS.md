...
## v3.38 增量段（2026-09-09）

**背景**：用户裁定执行 MODEL_LEVERAGE_EVAL_V3_37 评估件的 A-E 五候选——
模型能力利用的最大缺口在"对抗与实证的耦合"（Caddy 验收证明发现增量价值
几乎全部来自对抗与实证，而非生成量）。另修复 v3.37 两条目缺 binding 从不
挂载的隐性欠账（本批取证发现）。

**修复（SWR-V3.38-001~005）**：
1. 清单挂载修复 + CK-SIBLING-CONSISTENCY（同族不一致防御形态机制化——
   /config 封顶 /load 无的差分声称强于绝对声称；关键词按真实候选 summary
   词汇校准，不短路 resource 兜底 SWR-087）；
2. 自证伪轮结构化：VERDICT_SCHEMA 增可选 self_refutations（verifier 自认
   判定翻转点），证伪者以此为优先攻击面（从"找角度"变"攻前提"）；
3. 公开面关联检索前置 R2（CVE/GHSA 关联 + 上游 master 对账；时间差假设=
   上游后修即快照缺陷候选；网络不可用零阻塞）；
4. 实证机会条款（可构建 + 实证类声称 → 低成本实测建议，无环境记录 blocker；
   R3 实测数字是分级升档与证伪攻击面）；
5. harness 回收条款（收官复盘通用化评审 → 去项目化入库）。
