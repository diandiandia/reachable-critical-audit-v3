...
> 事实来源：W6_MORE_LANGS_FINDINGS.md §18（Dubbo 批次，Java 首审）与 §21.4（网络可达性）、
> SKILL_LESSONS_JAVA.md（fastjson2 三版本交叉复核）、SKILL_LESSONS_JAVA_dubbo.md（Dubbo 3.3）。
...
- 依赖库 sources jar 可获取：fastjson2 2.0.62 sources jar 验证 autoTypeBeforeHandler 接线（§18.2）。
- 真实解码器实证模式：匿名子类调用真实 `ExchangeCodec.decode`（§18.5）。
- 审计目标版本/HEAD 记录：Dubbo 3.3 branch HEAD `3a3043227f`（SKILL_LESSONS_JAVA_dubbo §0）。

## 2. 版本记录义务

- 依赖库语义必须落到版本级验证：fastjson2 2.0.62/2.0.63/2.0.65-dev 三版本交叉复核（SKILL_LESSONS_JAVA §0）；autoTypeBeforeHandler 接线位置（JSONReader.Context.config:5996-6001、JSONReaderJSONB:755-764 等三处解析点）按 2.0.62 核验（§18.2）。
- 内建 deny list 内容按版本记录：checkAutoType denyList 仅 ClassLoader 子类 + SQL DataSource/RowSet，TemplatesImpl/c3p0/spring/HikariCP 不在 deny 内（SKILL_LESSONS_JAVA §1.4）。
...
- sources_regex 只有 Web 注解（@RequestBody/HttpServletRequest），Dubbo 真实输入面是 Netty——源模型与项目输入面不匹配（SKILL_LESSONS_JAVA_dubbo §2）。
...
- R1 大代码库超时第二例：Dubbo 2400 文件 4 域测绘总耗时 2 小时+（§18.6）。
- 行号漂移量大：30+ 漂移（§18.7，对比 vapor 3 处手修）。
- 反序列化 gate 枚举不全：GenericFastJsonRedisSerializer() 无参构造仍默认 SupportAutoType——默认 Spring 配置无白名单 + autoType 开（SKILL_LESSONS_JAVA §1.4）。
- 规模管理：Dubbo 15,620 候选 173 验证批次；sources_regex 只有 Web 注解导致源模型与真实输入面（Netty）不匹配（SKILL_LESSONS_JAVA_dubbo §0/§2）。
...
- 反序列化 allowlist/checker 三态门控审计：DefaultSerializeClassChecker 的 checker/allowlist 类安全控制按三态（allow/deny/未知）核验（SKILL_LESSONS_JAVA_dubbo §5）。
- LOGIC_PATTERN 危险谓词规则：`TypeUtils.getMapping` 路径无 deny 检查、`checkAutoType` hash 白名单绕过（SKILL_LESSONS_JAVA §1.4/§2）。
