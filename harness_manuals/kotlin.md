...
> 事实来源：W6_MORE_LANGS_FINDINGS.md §19（Kotlin 首审批次）与 §21.4（网络可达性）。
...
- 依赖 jar 从 Maven Central 直取：按审计目标锁定版本（§19.2/19.4/21.4）。
- 目标 jar 与树源码字节码一致性核验是标准步骤（§19.4）。
- JDK 版本探测：解压/压缩类实证先读目标代码的 Inflater 参数再定 harness（§19.3）。

## 2. 版本记录义务

- 依赖锁版本精确记录：项目锁定版本（§19.2）、服务器框架 jar（§19.4）。
...
- 多引擎版本差异记录：内置引擎 vs 第三方引擎（第三方引擎用宽松 cookie 解码器重写）——REACHABLE 需在文档默认引擎成立（§19.8）。
...
- 多引擎"任一引擎成立"误判：PATH A 仅内置引擎执行；按旗舰引擎复核才定（§19.8）。
- 默认值=无防护族收割：帧长上限 Long.MAX / 解压内容上限 -1 / cookie secure false / 客户端 cookie 无签名 / 异常回显（§19.7）。

## 4. 阳性模式（战役验证过的做法）

- **verbatim 循环提取实证 + 真实依赖 jar**（T1b）：IO readBuffer 循环逐字符提取 + 锁定版本真实 Buffer——100MB 物化实证，绕开 gradle/expect-actual 构建墙（§19.2）。
- **真实 jar 对抗实证**：下载服务器框架 jar 跑畸形头矩阵，0/N 触发——"sink 分支是行为死代码"一击致命（§19.4）。
- 解压实证先读目标代码 Inflater 参数：目标源码用 `Inflater(true)` + 手动 GZIP_HEADER_SIZE 跳过（§19.3）。
- 类型系统先于配置检查：receiveType 编译期静态 + kotlinx 多态仅开发者注册类集 → 比运行时白名单更强的阻断（§19.6）。
- H7 数值上限类默认值专项：MAX_VALUE/-1/0 三值即红旗（对照正向例：URL 编码表单解析深度限制 100，§19.7）；H7 9 findings 全部"默认值=无防护"族（帧长上限 Long.MAX / 解压内容上限 -1 / cookie secure false / 客户端 cookie 无签名 / 异常回显）。
- 对照 check：同一个"框架流式 vs 应用物化"区分跨框架结论相反（物化引擎头部解析即 allocate 2GiB vs 流式引擎逐 chunk 交付，§20.2 跨语言对照）。

## 5. 网络依赖

- repo1.maven.org 可达（§21.4）——锁定版本 jar 直取成功。
- github.com 可达（§21.4）。
