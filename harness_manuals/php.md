...
> 事实来源：W6_MORE_LANGS_FINDINGS.md §17（WordPress 批次，PHP 首审）与
...
- 记录审计目标精确版本（WordPress 7.0.4 core，§17；phpMyAdmin 4.8.5 ground truth=CVE-2018-12613，SKILL_LESSONS_PHP §1.1）。
- 矩阵型前提必须随版本记录：theme-compat 的"Editor 角色 + 经典主题"是核心一线支持配置（§17.6）。
- 分层防御是动态注册的（kses_init / capability 条件注册）——防御链结论必须落到当前版本的实际注册点（§17.5）。

## 3. 常见陷阱清单

- LLM 假设生成 agent 失控：WordPress（>200k 行、48 面）上无限扩张验证范围 2.5 小时无产出；主代理接手 20 分钟产出 10 条真实行号假设（§17.1）。
...
- 签名规则框架绑定噪声：CWE-918 regex 把全部 PHP 文件系统函数当 SSRF（WordPress 命中 1246 个）；semgrep-registry 的 php.lang.security 规则绑定 Laravel/Doctrine，对 WP 的 `array->get()`/`sprintf()` 误报 14452 个（SKILL_LESSONS_PHP §4）。
...
- 矩阵类裁决按"最常见成立格"裁决：Editor+经典主题矩阵保留 REACHABLE 且端到端矩阵记录（§17.6，与 Django ALLOWED_HOSTS 先例 §14.7 合并）。
...
- 矩阵类裁决按"最常见成立格"裁决并显式记录矩阵（theme-compat Editor+经典主题，§17.6；与 Django ALLOWED_HOSTS 先例 §14.7 合并）。
- 分层动态防御框架（WordPress）的 UNREACHABLE 判定必须过一遍动态注册搜索（§17.5）。
