...
> 来源：W6_MORE_LANGS_FINDINGS.md（§14）。实证工件：审计项目内 `.audit_results/empirical_harness_t1.py`、`empirical_harness_t2.py`。

## 1. 工具链探测
- **纯 Python 项目实证环境成本最低**：`venv` + `sys.path.insert(0, '<audit-src>')` 直接导入审计源码即可跑全链路；Web 框架批次实测 `框架配置.configure(...)` + `框架.setup()` 最小运行时启动真实请求处理器（W6 §14.8 + t1 harness 工件）——无需安装目标包。
...
## 2. 版本记录义务
- 记录 python3 精确版本 + 审计目标版本 + 依赖版本（按 venv 实际版本）。
...
## 3. 常见陷阱清单
- **通用 regex 签名零区分度**：11 条 Python 规则 1784 命中 → 81 假设 0 keep——命中全是防御性转义（escape/re.escape/base64）、框架自配置、有界结构；真实 Web 框架 sink（请求体接收无界循环、Host 获取与校验、反序列化多后端族、路径拼接校验）零签名覆盖——通用规则（append/encode）应降权或退役，补框架语义族（请求体接收循环落盘 / Host 获取+校验 / 反序列化+cache、会话后端矩阵 / 模板标签正则 / 配置导入族）（W6 §14.1）。
- **LLM 假设生成器的防御性偏差**：23 假设中 13 条是"防御验证签收"（gate 已 Read 验证后建议不投入）——假设必须指向残余面，签收类假设不占 R3 队列，且防诱导 verifier 复读（W6 §14.2）。
- **"检查点晚于累积点"**（H1 清单第一条）：请求体接收循环全量落盘后 `_check_data_too_big` 才可触发 + 接收超时死代码——T1 实证 10,000× 超额；写循环内无预算检查是该族红旗（W6 §14.3，三次复现模式）。
...
- **成熟框架的假设产出天花板**：Web 框架批次 10 候选 → 2 REACHABLE + 1 降级，R3.5 拦截（开发服务器面、Host 白名单矩阵裁决）——成熟框架的 R3 假设应预设"机制真 ≠ 危害真"，verifier 对默认值类声称系统偏乐观（§20.1/§21.5 通用先验）。

## 4. 阳性模式（战役验证过的做法）
- **最小真实运行时模板**（可复制）：`sys.path.insert(0, audit_src)` + `框架配置.configure(DEBUG=False, 主机白名单=[...], 上传限额=<缩小>)` + `框架.setup()` → 直接实例化真实请求处理器类跑攻击载荷（t1 harness 工件）——**跑真实框架代码而非复刻逻辑**（§18.5 精神）。
- **声称类一律实证**：纯 Python 实证成本接近零（venv + sys.path）——Python 项目的所有声称类都应按此执行（W6 §14.8）。
- **verifier/R4 自发小实证被鼓励**：verifier 跑真实框架 checkout 验证 stdlib policy fail-closed（CAND-010 邮件头 CRLF）；R4 agent 实证 O(n²)（模板标签正则 60KB>60s）——实证结果落 verdict.empirical 字段，即使结论是 UNREACHABLE（W6 §14.5）。
- **subprocess 起真实服务测协议面**：runserver 的 414/close 行为、跨请求边界残留验证（T2 对照）用真实进程（W6 §14.4）。
- **全链路导入验证**：框架 `setup()` 触发所有信号/过滤链注册（§17.4-17.5 的动态注册教训在 Python 侧对应 setup 后状态）——harness 必须走完整 setup 而非 import 局部模块。
...
- **对照矩阵必带**：默认配置拒绝 vs 弱化配置接受（如上传限额缩小 + 默认限额两组测量）——§24.4 模式在 Python harness 直接可用。
- 分层：函数体级（模板标签正则 O(n²)）→ 机制级（handler 单例注入）→ E2E（真实框架 checkout + 开发服务器）按声称强度选档，报告标注 scope（§15.6 规则）。
