...
## 语义轴测绘段（v3.17, SWR-V3.17-005 —— 仅 surface_model=semantic/hybrid 目标注入）

运行时/引擎形态目标的真实输入面是目标实现的"语言"语义本身，入口点枚举
会退化为 parser/wasm 级粗粒度面。本节要求额外测绘**语义轴**（一轴一条
surface，entry_points 仍附 file:line 证据）：

- 轴 = 目标语言的语义命名空间族：内建函数族 / 字节码指令族 / 语法产生式族 /
  序列化格式构造族 / 配置指令族（按目标形态选取，枚举为主代理派发时给定）；
- 每条轴 surface 必填 `semantic_axis` 字段：
  `{"namespace":"该族名称","anchor_files":["file:line", ...],"cardinality":"估值(数量级)"}`；
...
- 根因归属条款（v3.24, SWR-V3.24-002）: 缺陷根因无法在 runtime 实现与编译器正确性之间归属时，标注归属未知并保持 [ambig]——不得默认归编译器正确性; CWE-843 映射仅适用于归属成立的候选。

## 组件约束段（v3.17, SWR-V3.17-002 —— 仅 super-large 两阶段测绘注入）

本目标为 super-large 档（源文件 >2000），R1 采用两阶段测绘。你负责的
组件为 **{component_scope}**（主代理派发时替换；未替换则按整个目标执行）：
- 只探测该组件目录内的面，面 id 带组件缩写前缀（SURF-<域>-<组件缩写>-NNN）；
- 产出写 `.audit_results/_r1_<域>_<组件缩写>.json`（合并时按前缀归并）；
- 45 分钟硬时限按组件给——时限内未完成时落盘中间产物并在最终回复标注
  `PARTIAL: <已覆盖子目录清单>`，禁止静默丢弃。
