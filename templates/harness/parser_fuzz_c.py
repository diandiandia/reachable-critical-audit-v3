...
有状态 sink 的最小 stub 复刻法 (v3.10, SWR-V3.10-012):
本模板的随机主循环适配无状态解析器; sink 依赖解析上下文/内核结构体时,
按以下纪律手工构造最小 stub 后再接入本模板编译:
- **无符号下溢语义保留**: 长度/指针字段的类型宽度 (u16/u32/size_t) 与
  下溢行为必须与目标代码逐字一致——stub 用同宽度类型复刻自减/pull 语义,
  不得改成有符号或加断言 (断言会掩盖目标缺陷)
- **边界指针语义**: 头部/尾部/数据指针的相对移动 (pull/trim/offset 步进)
  以独立指针字段复刻, 分配起点与指针偏移的布局比例按真实结构模拟
- **分配布局模拟**: 用 malloc 模拟目标分配单元 (页/块), 数据起点放在
  真实 headroom/头结构偏移处——越界判定依赖布局比例, 随意放大缓冲区会
  使越界不可观测
- **逐字提取纪律**: 提取的函数体保持逐字 (含注释来源行), stub 部分单独
  标注 "stub (语义复刻)" 与目标语义的行号来源——混写或改写会破坏证据链
